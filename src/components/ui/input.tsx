...
const STRENGTH_LEVELS = [
  ...
  { score: 2, key: 'fair',       barClass: 'bg-amber-500',   textClass: 'text-amber-500'   },
  { score: 3, key: 'good',       barClass: 'bg-yellow-500',  textClass: 'text-yellow-500'  },
  { score: 4, key: 'strong',     barClass: 'bg-emerald-500', textClass: 'text-emerald-500' },
  { score: 5, key: 'veryStrong', barClass: 'bg-emerald-600', textClass: 'text-emerald-600' },
] as const;
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, icon: Icon, showPasswordToggle, onChange, value, ...restProps }, ref) => {
                  ...
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div
                      ...
                      className={cn(
                        ...
                        i < strength.score ? strength.barClass : 'bg-muted/40',
                      )}
                    />
                  ))}
                </div>
                <p className={cn('text-xs font-semibold', strength.textClass)}>
